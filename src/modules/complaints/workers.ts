...
import {
	COMPLAINT_RECEIPT_DELIVERY_EVENT,
	COMPLAINT_RESPONSE_DELIVERY_EVENT,
	getComplaintDeliveryFailure,
	getComplaintDeliveryState,
	sendComplaintReceiptDelivery,
	sendComplaintResponseDelivery,
	setComplaintDeliveryStatus,
} from './delivery'

export const processComplaintReceiptDelivery = inngest.createFunction(
	{
		id: 'complaints-process-receipt-delivery',
		retries: 0,
		triggers: [{ event: COMPLAINT_RECEIPT_DELIVERY_EVENT }],
	},
	async ({ event, step }) => {
		const state = await step.run('load-receipt-delivery-state', () =>
			getComplaintDeliveryState({
		...
		if (state.receiptDeliveryStatus === 'sent') {
			return { ok: true, skipped: true }
		}

		await step.run('mark-receipt-delivery-processing', () =>
			setComplaintDeliveryStatus({
				complaintId: event.data.complaintId,
				organizationId: event.data.organizationId,
				workflow: 'receipt',
		...
		try {
			...
			await step.run('mark-receipt-delivery-sent', () =>
				setComplaintDeliveryStatus({
					complaintId: event.data.complaintId,
					organizationId: event.data.organizationId,
					workflow: 'receipt',
			...
			await step.run('mark-receipt-delivery-failed', () =>
				setComplaintDeliveryStatus({
					complaintId: event.data.complaintId,
					organizationId: event.data.organizationId,
					workflow: 'receipt',
			...
			await step.run('log-receipt-delivery-failed', () =>
				createAuditLog({
					organizationId: event.data.organizationId,
					action: AUDIT_LOG.COMPLAINT_RECEIPT_DELIVERY_FAILED,
