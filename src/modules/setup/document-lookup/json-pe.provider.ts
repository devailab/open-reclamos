...
interface JsonPeRucResponseData {
	ruc: string
...
interface JsonPeRucResponse {
	success: boolean
	data: JsonPeRucResponseData
}

export class JsonPeProvider implements DocumentLookupProvider {
	constructor(
	...
	async lookupRuc(ruc: string): Promise<RucData> {
		...
		const json: JsonPeRucResponse = await response.json()
